...
class LED:
    ...
    def send_message(self, message):
        # 사용자 입력 메세지 전송
        text_data = ProtocolPacket.send_text(self, Data=message.encode(latin-1))  # 메세지 인코딩
        send_text = ProtocolPacket.packet_start(self) + text_data + ProtocolPacket.packet_end(self)
        self.serial.write(send_text)
        print(f"LED 전광판 버퍼에 메세지 '{message}' 를 보냈습니다.")
...
if __name__ == "__main__":
        ...
        LED.send_message()
        # 사용자에게 메세지 입력 받기
        message = input("LED 전광판에 표시할 문구 입력: ")
        time.sleep(1) # 1초 대기
